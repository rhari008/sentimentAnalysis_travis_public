...
if( arch === 'ppc64' ) {
	...
	if( endianer === 'le' ) {
		modPath = platform+ '-'+ arch + endianer + '-v8-'+ v8;
	}
}
if (!force) {
	try {
		fs.statSync(path.join(__dirname, 'bin', modPath, 'jwt.node'));
...
function afterBuild() {
	var targetPath = path.join(__dirname, 'build', debug ? 'Debug' : 'Release', 'jwt.node');
	var installPath = path.join(__dirname, 'bin', modPath, 'jwt.node');
