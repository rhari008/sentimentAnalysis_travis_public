...
var dbSegments = require('./../db/dbSegment');
var sel = require('./expandSelectTreeBuilder');

var Http404_NotFound = require('./../utils/errors/http/notFound');
var Http400_BadRequest = require('./../utils/errors/http/badRequest');
var associationUtils = require('./../utils/associations');
...
ResourcePathReader.prototype.consumeFirstSegment = function consumeFirstSegment() {
    ...
    // check if this segments is used to supply input paramters for the following calcview segment
    if (entityType.kind === model.entityKind.inputParameters) {
        ...
        if (!segment) {
            throw new Http400_BadRequest('Navigation to calc view expected after an Input path segment');
        }
    ...
    if (segment.keys) {
        this.firstDBSegment.setKeyValues(segment.keys); // set key array oder single key value
        this.firstDBSegment.isCollection = false;
    } else {
        this.firstDBSegment.isCollection = true;
    }
